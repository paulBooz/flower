...
```
Disclaimer: Please note that since the experiments were conducted for the paper, the dataset authors have slightly modified the dataset. While this modification leads to differences in accuracy (~10%), `StatAvg` should still consistently outperform `FedAvg` as demonstrated in the paper.
